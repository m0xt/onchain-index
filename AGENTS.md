...
| `scripts/com.milkroad.onchain-index-refresh-daily.plist` | Legacy weekday launchd template; Hermes cron `onchain-index-refresh-daily` is the daily 22:30 Prague dashboard/docs refresh job. |
...
- Cron path: `scripts/refresh.sh` (Hermes cron `onchain-index-refresh-daily`, every day 22:30 Prague, logs to `.cache/refresh.log`.)
